...
/* Purpose: Checks to see if a root node's left and right node values are correct to be a binary tree
  / @param: root node
  / @return: NULL if tree is correct or the node of where its left and right nodes are not correct
  */
Node* isValidBSTHelper(Node* currNode) {
  if (currNode != NULL) {
    // Check if current node's value is greater than left node
    if (currNode->left != NULL) {
      if (currNode->key <= currNode->left->key) {
        return currNode;
      }
    }
    if (currNode->right != NULL) {
    ...
    isValidBSTHelper(currNode->left);
    isValidBSTHelper(currNode->right);
  }
}
